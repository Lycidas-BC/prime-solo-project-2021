      ...
      .then((response) => {
        if (productUrl.toLowerCase().includes("criterion.com")) {
          if (productUrl.toLowerCase().includes("/films/")) {
            res
              .status(201)
              .send(scrapeCriterionFilmData(response.data, productUrl));
          } else if (productUrl.toLowerCase().includes("/boxsets/")) {
            res
              .status(201)
              .send(scrapeCriterionSetData(response.data, productUrl));
          } else {
            res
              .status(400)
              .send(
                "Send me that link and I'll try to figure out why it didn't work"
              );
          }
        } else {
          res
            .status(400)
            .send("web scraping only enabled for certain distributors");
        }
      })
      .catch((err) => {
        console.log(`error scraping product information from website`, err);
