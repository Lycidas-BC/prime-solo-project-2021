...
function AddMedia() {
  ...
  const addToCollection = () => {
    ...
    if (
      mediaItem.item !== "" &&
      mediaItem.movieList.length !== 0 &&
      (!displayScrapeProgress || mediaItem.progress === "COMPLETE")
    ) {
      dispatch({
        type: "ADD_MEDIA_ITEM",
        payload: mediaItem,
  ...
  return (
    <section
      style={{ margin: "5%" }}
      onMouseMove={() => setDisplayWarning(false)}
    >
      <div style={{ color: "red" }}>
        <Button
          style={{ width: "150px", height: "55px", marginBottom: "20px" }}
          variant="contained"
          color="primary"
      ...
      <div>
          ...
          {editArt ? (
            ...
            <div style={{ float: "left", width: "25%", marginRight: "10px" }}>
              <div style={{ display: "inline" }}>
                <img
                  src={mediaItem.cover_art}
                  alt={`${mediaItem.item} cover`}
                />
                <Button onClick={() => handleEditCoverArt()}>
                  <EditIcon />
                  Edit Cover Art?
                </Button>
              </div>
        ...
        <section style={{ clear: "both" }}>
          ...
          {addMovie ? (
            <div>
              <div>
                <Button onClick={() => setAddMovie(!addMovie)}>
                  <RemoveIcon />
                </Button>
              </div>
              <SearchPage
            ...
            <section>
              ...
              <div
                ...
                onDoubleClick={() => setAddMovie(!addMovie)}
              >
                <MovieItem
                  movieIn={"none"}
                  addMovieScreen={true}
        ...
        <section className="specialFeatures">
          ...
          <ul>
            ...
            {addFeature ? (
              <li>
                <div>
                  <Button onClick={() => setAddFeature(!addFeature)}>
                    <RemoveIcon />
                  </Button>
                </div>
                <TextField
                  style={{ width: "400px" }}
                  id="specialFeatureDescription"
                  label="description"
                  type="specialFeatureDescription"
                  variant="outlined"
                  value={newFeature}
                  onChange={(event) => setNewFeature(event.target.value)}
                />
                <Button onClick={() => addFeatureToList()}>
                  <DoneIcon />
                </Button>
              </li>
            ) : (
