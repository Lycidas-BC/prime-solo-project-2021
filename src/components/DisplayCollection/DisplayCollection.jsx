...
function DisplayCollection() {
  ...
  useEffect(() => {
    dispatch({
      type: "GET_COLLECTION",
      payload: { order: `${orderBy} ${direction}` },
    });
  }, []);
  ...
  return (
    <section>
      <section
        style={{ alignItems: "flex-end", display: "flex", flexWrap: "wrap" }}
      >
      ...
      <section
        className="media"
        style={{ alignItems: "flex-start", display: "flex", flexWrap: "wrap" }}
      >
        ...
        {mediaCollection.map((element, index) => {
          return element.columnHeaders === undefined ? (
            <DisplayCollectionItem
              key={index}
              mediaIn={element}
              addMedia={false}
