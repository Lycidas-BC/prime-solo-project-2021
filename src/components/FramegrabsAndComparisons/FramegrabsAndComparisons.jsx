...
function BrowseSearchResults() {
  ...
  const getMovieDetails = () => {
    ...
    history.push(`/search/1/${framegrabReducer[0].content_type}/${framegrabReducer[0].tmdb_id}`);
  };
  ...
  const organizeFramegrabLists = () => {
    ...
    for (const movieObject of framegrabReducer) {
      maxLength = (movieObject.framegrab_list.length > maxLength ? movieObject.framegrab_list.length : maxLength);
    }
    ...
    for (let i = 0; i < framegrabReducer.length; i++) {
      for (let j = 0; j < maxLength; j++) {
        if (j < framegrabReducer[i].framegrab_list.length && framegrabReducer[i].framegrab_list[j] != null) {
          combinedObjectArray.push({
            ...
            pretty_timestamp: `${new Date(Number(framegrabReducer[i].framegrab_list[j].split(" ")[1]) * 1000).toISOString().substr(11, 8)}`
          })
        }
      }
    }
    ...
    // sort
    combinedObjectArray = combinedObjectArray.sort((a,b) => (a.timestamp > b.timestamp) ? 1 : ((b.timestamp > a.timestamp) ? -1 : 0));
  ...
  return (
    ...
    <section>
      <h1>
        ...
        <Button style={{marginLeft: "20px"}} onClick={getMovieDetails}><MoreHorizIcon />Get Movie Details</Button>
        <Button style={{marginLeft: "20px"}} onClick={() => toggleArt("media")}>{showItemArt ? "Hide media cover art" : "Show media cover art"}</Button>
        <Button style={{marginLeft: "20px"}} onClick={() => toggleArt("movie")}>{showMovieArt ? "Hide media movie art" : "Show media movie art"}</Button>
        <Button style={{marginLeft: "20px"}} onClick={() => {organizeFramegrabLists(); setShowFramegrabs(!showFramegrabs)}}>{showFramegrabs ? "Hide framegrabs" : "Show framegrabs"}</Button>
      </h1>
        ...
        {framegrabReducer.map((element, index) => {
          ...
          <StyledTableRow  key={index}>
            <StyledTableCell align="left" width="300px" >
              <Button onClick={() => mediaDetailsScreen(element.media_id)}><MoreHorizIcon /></Button>{element.item}
              {showItemArt ?
                <div>
                  <CardMedia
            ...
            <StyledTableCell align="left"><a href={element.product_page}>{element.distributor}</a></StyledTableCell>
            <StyledTableCell align="left">{element.format}</StyledTableCell>
            <StyledTableCell align="left">{`${new Date(Number(element.length)*60*1000).toISOString().substr(11, 5)}`.replace(":", "h ")}m</StyledTableCell>
            <StyledTableCell align="left">
              {addFramegrabSelector === element.media_movie_id ? 
              <section>
                <div>
                  <Button onClick={() => setAddFramegrabSelector(-1)}><RemoveIcon /></Button>
                  <Button onClick={() => uploadFramegrab(element.media_movie_id)}><SaveIcon /></Button>
                </div>
                <div>
    ...
    {showFramegrabs ? 
      framegrabDataArray.length === 0 ? <div>No associated framegrabs</div> :
    <div>
      {framegrabDataArray.map((element,index) => {
        return (
          <section key={index} style={{marginBottom: "25px"}}>
            <img src={element.framegrab_url} style={{maxWidth:"960", maxHeight:"530"}}></img>
            <div>({element.distributor} {element.format}) {element.name} from edition {element.name} at timestamp {element.pretty_timestamp}</div>
          </section>
        )
      })}
    </div>
