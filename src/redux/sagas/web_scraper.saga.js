...
function* scrapeWebsite(action) {
  try {
    let scrape = yield axios.get(
      `/web_scraper/scrapeProductPage/?productUrl=${encodeURIComponent(
    ...
    for (const index in scrape.data.movieList) {
      ...
      const searchType = scrape.data.movieList[index].description.match(
        /television/i
      )
        ? "multi"
        : "movie";
      // if there's a ":" in the item name, exclude what's after
      const searchTerm = scrape.data.movieList[index].movie.substring(
        0,
        scrape.data.movieList[index].movie.indexOf(":") === -1
          ? scrape.data.movieList[index].movie.length
          : scrape.data.movieList[index].movie.indexOf(":")
      );
      ...
      while (page <= total_pages && !endWhileLoopNow) {
        const searchResults = yield axios.get(
          `/api/tmdb/searchSpecific/?type=${encodeURIComponent(
            searchType
          )}&q=${encodeURIComponent(searchTerm)}&year=${encodeURIComponent(
            scrape.data.movieList[index].year
          )}&page=${encodeURIComponent(page)}`
        );
        if (searchResults.data.tmdb_id.length === 1) {
          scrape.data.movieList[index].tmdb_id = searchResults.data.tmdb_id[0];
          scrape.data.movieList[index].media_type =
            searchResults.data.mediaTypeList[0];
      ...
      const scrapeLength = yield axios.get(
        `/web_scraper/scrapeLength/?productUrl=${encodeURIComponent(
          scrape.data.movieList[index].product_url
        )}`
      );
