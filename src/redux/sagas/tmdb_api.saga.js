...
function* getTmdbDetails(action) {
  try {
    const tmdbResponse = yield axios.get(
      `/api/tmdb/details/${
        action.payload.tmdbId
      }/?searchType=${encodeURIComponent(action.payload.searchType)}`
    );
    let tmdbDetails = tmdbResponse.data;
    console.log(tmdbDetails);
    const mediaIdList = yield axios.get(
      `/media_collection/searchCollection/${encodeURIComponent(
        tmdbResponse.data.tmdbIdList
      )}`
    );
...
function* getTmdbStreamingOptions(action) {
  try {
    const streamingOptions = yield axios.get(
      `/api/tmdb/streamingOptions/${
        action.payload.id
      }/?tvOrMovie=${encodeURIComponent(action.payload.tvOrMovie)}`
    );
    console.log("streamingOptions:", streamingOptions);
    yield put({
      type: "SET_TMDB_STREAMING_OPTIONS",
      payload: streamingOptions.data,
    });
  } catch (error) {
...
function* searchTmdb(action) {
  try {
    ...
    const searchResults = yield axios.get(
      `/api/tmdb/search/?type=${encodeURIComponent(
        action.payload.type
      )}&q=${encodeURIComponent(
        action.payload.query
      )}&page=${encodeURIComponent(action.payload.page)}`
    );
